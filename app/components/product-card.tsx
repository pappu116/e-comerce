...
export default function ProductCard({ product }: { product: any }) {
  ...
  // প্রোডাক্টটি উইশলিস্টে আছে কি না চেক করা
  ...
  return (
    <Link href={`/product/${product.id}`} className="block group">
      <motion.div 
        whileHover={{ y: -8 }}
        className="relative rounded-[2rem] md:rounded-[2.5rem] border border-white/10 bg-white/[0.03] p-3 md:p-4 transition-all hover:shadow-[0_20px_40px_rgba(0,0,0,0.4)] hover:border-indigo-500/40 backdrop-blur-md overflow-hidden"
      >
        ...
        >
          ...
          >
            <Heart 
              size={16} 
              className={`transition-colors md:w-[18px] md:h-[18px] ${isFavorite ? "fill-red-500 text-red-500" : "text-white"}`} 
            />
          </motion.div>
        </button>
        ...
        <div className="relative aspect-[4/5] sm:aspect-square overflow-hidden rounded-[1.5rem] md:rounded-[2rem] bg-zinc-900 shadow-inner">
          ...
          {/* Mobile Quick Add Button (Visible on hover for desktop, persistent small icon for mobile) */}
          <div className="absolute inset-0 bg-gradient-to-t from-black/60 via-transparent to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex items-end p-4 hidden md:flex">
        ...
        <div className="mt-4 px-1 md:px-2 space-y-2">
          <div className="flex justify-between items-start gap-2">
            <h3 className="font-black text-slate-100 text-sm md:text-lg leading-tight group-hover:text-indigo-400 transition-colors line-clamp-1 md:line-clamp-none">
              {product.name}
            </h3>
          </div>
          ...
          <div className="flex items-center justify-between gap-2">
            <div className="flex flex-col">
              ...
              {product.oldPrice && (
                <p className="text-[9px] text-slate-500 line-through font-bold mt-1">৳{product.oldPrice}</p>
              )}
            </div>
          ...
          {/* New Arrival Tag - Repositioned for cleaner look */}
          <div className="pt-2">
